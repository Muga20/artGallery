...
function Medium() {
  ...
  useEffect(() => {
    const getMediums = async () => {
      try {
        const response = await api(`/medium/get_mediums?page=${page}&pageSize=${pageSize}`, "GET", {}, {});
        setMediums(response);
      } catch (error) {
  ...
  const handleDeleteMedium = async (id) => {
    try {
      ...
      setSuccessMessage("medium deleted")
    } catch (error) {
      setErrors("Error deleting medium:", error);
    }
  };
        ...
        <AdminNavbar open={open} setOpen={setOpen} loading={loading} setLoading={setLoading}/>
          ...
          <section className="px-6 py-6">
                  ...
                  <table className="w-full table-auto">
                    <thead>
                      <tr className="text-xs text-left text-gray-500 dark:text-gray-400">
                        ...
                        <th className="px-6 pb-3 font-medium ">Medium</th>
                        <th className="px-6 pb-3 font-medium"> </th>
                      </tr>
                    </thead>
                    <tbody>
                    {mediums.map((medium) => (
                      <tr key={medium.id} className="text-xs bg-gray-100 dark:text-gray-400 dark:bg-transparent">
                        <td className="px-6 py-5 font-medium">{medium.id}</td>
            ...
            <div className="flex justify-around w-40 pt-10">
                ...
                <button className="text-white cursor-pointer" onClick={() => setPage(page + 1)} disabled={mediums.length < pageSize}>
                  Next
                </button>
            </div>
