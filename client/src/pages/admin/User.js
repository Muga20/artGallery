...
function User() {
  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        const response = await api(`/user/get_users?page=${page}&pageSize=${pageSize}`, "GET", {}, {});
        setUsers(response);
      } catch (error) {
  ...
  const handleInActivateUser = async (id) => {
    try {
      ...
      if (response.success) {
        setUsers((items) =>
          items.map((item) =>
            item.id === id ? { ...item, isActive: "Inactive" } : item
          )
        );
      } else {
          ...
          <section className="bg-gray-100 dark:bg-gray-800">
            <section className="px-6 py-6">
                          ...
                          >
                            ...
                            <td className="px-6 py-5 font-medium ">
                             {user.roles.map((role, index) => (
                                <span key={index}>
                                  {role.role}
                                  {index !== user.roles.length - 1 ? ', ' : ''} {/* Add comma if not the last role */}
                                </span>
                              ))}
                            </td>
                            <td>
              ...
              <div className="flex justify-around w-40 pt-10">
                ...
                <button className="text-white cursor-pointer" onClick={() => setPage(page + 1)} disabled={user.length < pageSize}>
                  Next
                </button>
              </div>
