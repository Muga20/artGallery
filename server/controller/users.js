...
//     client.get(`user:${user_id}`, async (err, cachedUserData) => {
//       if (err) {
//         throw err;
//       }

//       if (cachedUserData) {
//         // If user data is found in the cache, parse it and send it as the response
//         const user = JSON.parse(cachedUserData);
//         res.json({ roleNames: user.roleNames, user });
//       } else {
...
//             `user:${user_id}`,
//             3600,
//             JSON.stringify({ roleNames, user })
//           );
//           // The above line sets the cache to expire after 1 hour (3600 seconds)

//           res.json({ roleNames, user }); // Sending only the role names in the response
//         } else {
//           res.status(404).json({ error: "User not found" });
//         }
//       }
